        ...
        <div className="dark:bg-slate-800">
            <div className="container mx-auto">
                <div className="flex flex-col items-center">
                    <MyComponent deckType={DECK_TYPE} currentPage={currentPage} perPage={PER_PAGE} initialData={data} />
                    <Pagination
                        totalCount={totalCount}
                        currentPage={currentPage}
                        perPage={PER_PAGE}
                        basePath="/deck-list/gardevoir"
                        onPageChange={() => { }}
...
export const getStaticPaths: GetStaticPaths = async () => {
    ...
    const filteredData = data.form.filter((item: FormDataItem) => item.decktype === DECK_TYPE); // デッキタイプでフィルタリング
...
export const getStaticProps: GetStaticProps = async ({ params }) => {
    ...
    try {
        ...
        const pageData = filteredData.slice(startIndex, startIndex + PER_PAGE);
