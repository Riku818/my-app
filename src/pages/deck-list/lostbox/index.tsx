...
const LostBox: React.FC<HomeProps> = ({ totalCount, initialData }) => {
  return <>
    <main className="flex flex-col w-full dark:bg-slate-800">
      <h1 className="flex justify-center text-5xl text-slate-950 mt-32 mb-8 font-semibold dark:text-white">ロストバレットデッキ</h1>
      <div className="flex justify-center mt-16 mb-16">
        <div className="flex flex-col items-center">
          <MyComponent deckType="ロストバレット" currentPage={1} perPage={PER_PAGE} initialData={initialData} />
          <Pagination
            totalCount={totalCount}
            currentPage={1}
            perPage={PER_PAGE}
            basePath="/deck-list/lostbox"
            onPageChange={() => { }}
            indexPath="/deck-list/lostbox"
          />
